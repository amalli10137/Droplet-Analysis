...
start_time = 5  # Set the starting time index here
...
df.to_csv('neck_radius_vs_time.csv', index=False)
...
plt.savefig('neck_radius_vs_time_plot.png')
plt.show()
...
plt.xlabel('Time (Image Index)')
plt.ylabel('Neck Radius (pixels)')
plt.savefig('neck_radius_vs_time_loglog_plot.png')
plt.show()
...
plt.xlabel('Time (Image Index)')
plt.ylabel('Neck Radius (pixels)')
plt.savefig('neck_radius_vs_time_semilogx_plot.png')
plt.show()

# Create a new folder for the images with red lines
output_folder = 'processed_images_with_lines'
...
for i, img in enumerate(rotated_processed_cropped_img_collection):
    ...
    output_path = os.path.join(output_folder, f'image_with_line_{i+1}.png')
